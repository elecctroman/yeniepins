...
if (!function_exists('asset')) {
    function asset(string $path): string
    {
        ...
        if ($documentRoot && $publicDir && str_starts_with($publicDir, $documentRoot)) {
            ...
            if ($scriptDir !== '' && str_starts_with($segment, $scriptDir)) {
                $segment = trim(substr($segment, strlen($scriptDir)), '/');
            }
            $relativePublic = $segment;
            $detected = true;
        }

        if (!$detected && $relativePublic === '') {
            $relativePublic = 'public';
        }

        if ($relativePublic !== '' && !str_ends_with($base, '/' . $relativePublic)) {
            $base .= '/' . $relativePublic;
        }
