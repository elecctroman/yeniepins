...
5. `config/config.php` içindeki `app_key` ve `env` değerlerini ortamınıza göre güncelleyin.
6. Veritabanında sırasıyla `database/schema.sql` ve `database/seed.sql` dosyalarını çalıştırın.
7. `storage/` altındaki klasörlerin web sunucusu tarafından yazılabilir olduğundan emin olun.
