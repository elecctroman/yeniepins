...
class AccountStock extends Model
{
    ...
    {
        parent::__construct();
        $this->reservationFile = __DIR__ . '/../../storage/cache/account_reservations.json';
        if (!file_exists($this->reservationFile)) {
            file_put_contents($this->reservationFile, json_encode([], JSON_UNESCAPED_UNICODE));
        }
        $this->key = (string)\env('APP_KEY', 'yeniepins');
    }
    ...
    public function markAsSold(string $token): void
    {
        $reservations = $this->loadReservations();
        if (!isset($reservations[$token])) {
            return;
