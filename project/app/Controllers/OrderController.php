...
class OrderController extends Controller
{
    ...
    public function create(array $params = []): void
    {
        ...
        try {
            ...
            Logger::error('Sipariş oluşturulamadı: {message}', [
                'order_no' => $orderNo,
    ...
    public function paymentCallback(array $params = []): void
    {
        ...
        if (!$order) {
            ...
            Logger::warning('Sipariş ödeme callback bulundu: sipariş yok', ['order_no' => $orderNo]);
            return;
        }

        if (in_array($status, ['success', 'paid', 'completed'], true)) {
            $delivery = $this->finalizeOrder($order, $gateway, $paymentRef);
            Logger::info('Sipariş ödemesi onaylandı', [
                'order_no' => $orderNo,
                'gateway' => $gateway,
        ...
        Logger::warning('Sipariş ödemesi başarısız', [
            'order_no' => $orderNo,
            'gateway' => $gateway,
            'status' => $state,
        ]);
    ...
    public function adminRedeliver(array $params = []): void
    {
        ...
        if ($email) {
            ...
            Logger::info('Sipariş teslimatı tekrar gönderildi', ['order_no' => $orderNo]);
        } else {
    ...
    public function adminInitiateRefund(array $params = []): void
    {
        ...
        Logger::info('Admin iade süreci başlattı', ['order_no' => $orderNo]);
    ...
    protected function finalizeOrder(array $order, string $gateway, string $paymentRef): array
    {
        ...
        Logger::info('Sipariş teslim edildi', [
